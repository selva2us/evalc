...
# Third TOC convention: some books (e.g. older "For Dummies"-style titles)
# list their contents as "Part I - Title" / "Chapter 1 - Title" with NO
# dot-leaders and NO page numbers at all. PART_RE is the module-level line,
# CHAPTER_NODOTS_RE is the nested page-level line. Unlike MODULE_RE/PAGE_RE,
# these are only tried as a fallback (see parse_toc_entries_no_dots) because
# without page numbers there's no reliable signal to tell a real TOC line
# from a running chapter-breadcrumb header repeated on body pages.
PART_RE = re.compile(r"\A\s*Part\s+[IVXLCDM]+\s*[-:]\s*(?P<title>.+?)\s*\Z", re.IGNORECASE)
CHAPTER_NODOTS_RE = re.compile(r"\A\s*Chapter\s+\d+\s*[-:]\s*(?P<title>.+?)\s*\Z", re.IGNORECASE)
...
def parse_toc_entries_no_dots(per_page_texts: list[str]) -> list[dict]:
    """Fallback parser for the 'Part I - Title' / 'Chapter 1 - Title' TOC
    convention (no dot-leaders, no page numbers).

    Reads page texts in order and accumulates matches. Some PDFs of this
    style repeat the same Part/Chapter breadcrumb as a running header on
    every body page (and on this kind of PDF, that repeated text is often
    itself garbled/interleaved with body content past the first repeat).
    To stay safe against both a) a normal multi-page TOC and b) a
    repeated/corrupted breadcrumb, we stop as soon as a page's matches
    are non-empty and exactly reproduce everything already collected -
    that signals the real TOC is complete and we're now just seeing the
    header repeat (and possible corruption) on subsequent pages.
    """
    entries: list[dict] = []
    entries_key: list[tuple] = []  # (level, title) for equality checks
    ...
    for page_text in per_page_texts:
        ...
        for raw_line in page_text.splitlines():
            ...
            if m:
                page_entries.append({"level": "module", "title": clean_title(m.group("title"))})
                continue
            m = CHAPTER_NODOTS_RE.match(line)
            if m:
                title = clean_title(m.group("title"))
                if FRONT_MATTER_RE.match(title):
                    continue
                page_entries.append({"level": "page", "title": title})

        if not page_entries:
            if entries:
                break  # TOC region ended, body page with no breadcrumb matches
            continue  # haven't found the TOC yet, keep looking

        page_key = [(e["level"], e["title"]) for e in page_entries]
        if page_key == entries_key:
            # This page contributed nothing new - full repeat of what we
            # already have. Real TOC is complete; stop before any
            # corrupted/garbled repeats further into the document.
            break

        # Otherwise this page extends the TOC with genuinely new lines
        # (normal case for a multi-page real TOC in this convention).
        entries = page_entries if not entries else entries + [
            e for e, k in zip(page_entries, page_key) if k not in entries_key
        ]
        entries_key = [(e["level"], e["title"]) for e in entries]
...
# ---------------------------------------------------------------------------
# Pillar grouping: an LLM call decides *where the natural topic breaks fall*
# across the real chapter list, rather than chunking every N chapters into a
# pillar. Every chapter/page title stays exactly what's in the source PDF -
# only the grouping/labeling is model-assisted, so extraction later still
...
def _fallback_pillar_boundaries(num_chapters: int, modules_per_pillar: int) -> list[dict]:
    """Used only if the LLM grouping call fails or no API key is configured -
    same fixed-size chunking behavior as before, so the pipeline still runs."""
    return [{"title": None, "start_index": i} for i in range(0, num_chapters, modules_per_pillar)]


def group_chapters_into_pillars(modules: list[dict], cfg, logger) -> list[dict]:
    ...
    try:
        ...
        # Validate: strictly increasing start_index, first is 0, all in range.
        starts = [p["start_index"] for p in pillars]
        if starts[0] != 0 or starts != sorted(set(starts)) or any(
            s < 0 or s >= len(modules) for s in starts
        ):
            raise ValueError(f"Model returned invalid pillar boundaries: {starts}")

        logger.info("LLM grouped %d chapters into %d pillars based on subject scope",
                    len(modules), len(pillars))
        return pillars
    except Exception as e:
        logger.error("Pillar grouping via LLM failed (%s); falling back to fixed-size grouping", e)
        return _fallback_pillar_boundaries(len(modules), cfg.modules_per_pillar)


CHAPTER_SPLIT_THRESHOLD = 8  # only split a module into "Chapter N (Part K)" sub-groups
                              # if it has more real sections than this - most chapters
                              # in a typical reference book stay under this and don't
                              # need an artificial sub-split at all.


def chapters_for(pages: list[str], per_chapter: int) -> list[dict]:
    if len(pages) <= CHAPTER_SPLIT_THRESHOLD:
        return [{"number": 1, "pages": pages}]
    return [
...
def build_hierarchy(modules: list[dict], pages_per_chapter: int, pillar_boundaries: list[dict]) -> list[SkeletonPillar]:
    # Slice modules into contiguous groups using the given start indices.
    starts = [p["start_index"] for p in pillar_boundaries] + [len(modules)]
    pillars = []
    module_counter = 0
    for pillar_idx, (start, end) in enumerate(zip(starts, starts[1:]), start=1):
        mod_group = modules[start:end]
        boundary = pillar_boundaries[pillar_idx - 1]
        label = boundary.get("title") or (
            mod_group[0]["title"] if len(mod_group) == 1 else f'{mod_group[0]["title"]} to {mod_group[-1]["title"]}'
        )
        pillar = SkeletonPillar(number=pillar_idx, title=label)
        for mod in mod_group:
            module_counter += 1
            skel_mod = SkeletonModule(number=module_counter, title=mod["title"])
            for chap in chapters_for(mod["pages"], pages_per_chapter):
                chap_title = mod["title"] if len(chap["pages"]) == len(mod["pages"]) and chap["number"] == 1 \
                    else f'{mod["title"]} (Part {chap["number"]})'
                skel_chap = SkeletonChapter(number=chap["number"], title=chap_title)
...
def generate_skeleton(pdf_path: str, work_dir: Path, cfg, logger) -> dict:
    ...
    if not entries:
        logger.info("No dot-leader TOC found, trying 'Part/Chapter' fallback convention...")
        entries = parse_toc_entries_no_dots(toc_texts)
    if not entries:
        raise RuntimeError(
            "No table-of-contents entries found in either the dot-leader "
            "('Title ....... N') or 'Part I - Title' / 'Chapter 1 - Title' "
            "formats. This PDF may use a different TOC convention."
        )
    ...
    pillar_boundaries = group_chapters_into_pillars(modules, cfg, logger)
    pillars = build_hierarchy(modules, cfg.pages_per_chapter, pillar_boundaries)
